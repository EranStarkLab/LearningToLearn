% LtL_CALC_BIAS_PT          calculate an analytical estimate of MI bias from a joint probability table.
%
% call                  BIAS = LtL_CALC_BIAS_PT( MAT, MODE )
%
...
function bias = LtL_calc_bias_PT( mat, mode )
...
switch mode
    ...
    case { 2, 'bayes' } % supposed to be optimal
        R = LtL_bayescount( sum( sum( mat ) ), sum( mat, 2 ) / sum( sum( mat ) ) );
        nz = sum( mat );
        R_s = zeros( 1, S );
        for s = 1 : S
            if nz( s )
                R_s( s ) = LtL_bayescount( sum( mat( :, s ) ), mat( :, s ) / sum( mat( :, s ) ) );
            end
        end
        sumR_s = sum( R_s );
